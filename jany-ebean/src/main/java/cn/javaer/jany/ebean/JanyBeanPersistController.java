...
public class JanyBeanPersistController extends BeanPersistAdapter {
    ...
    public boolean preUpdate(BeanPersistRequest<?> request) {
        ...
        Set<UnsetInfo> unsets = unsetProps.computeIfAbsent(request.bean().getClass(), (k) -> {
            final Field[] fields = FieldUtil.getFields(request.bean().getClass());
        ...
        for (UnsetInfo info : unsets) {
            ...
            requestBean.intercept().setPropertyLoaded(info.getFieldName(),
                    !(ObjUtil.isEmpty(newValue) && info.isOnlyEmpty()));
        }
        return true;
    }
}
